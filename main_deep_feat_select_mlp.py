...
"""
An example of running deep feature selection (based on MLP) 
for only a few number of hidden layers.
...
for i in range(len(lambda1s)):       
    classifier,training_time=deep_feat_select_mlp.train_model(train_set_x_org=train_set_x_org, train_set_y_org=train_set_y_org, 
                          valid_set_x_org=valid_set_x_org, valid_set_y_org=valid_set_y_org, 
                          learning_rate=0.1, alpha=0.1, lambda1=lambda1s[i], lambda2=1.00,
                          alpha1=0.0001, alpha2=1, n_hidden=[128,64],
                          n_epochs=1000, batch_size=100, activation_func="tanh", rng=rng)
    ...
    test_set_y_pred=deep_feat_select_mlp.test_model(classifier, test_set_x_org, batch_size=200)
