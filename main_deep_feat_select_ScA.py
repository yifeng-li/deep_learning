...
"""
An example of running deep feature selection for many number of hidden layers
based on stacked contractive autoencoder.

@author: yifeng
"""
import os
import numpy
import deep_feat_select_ScA
...
for i in range(len(lambda1s)):       
    classifier,training_time=deep_feat_select_ScA.train_model(train_set_x_org=train_set_x_org, train_set_y_org=train_set_y_org, 
                valid_set_x_org=valid_set_x_org, valid_set_y_org=valid_set_y_org, 
                pretrain_lr=0.1,finetune_lr=0.1, alpha=0.01, 
                lambda1=lambda1s[i], lambda2=1, alpha1=0.0001, alpha2=0.5,
                n_hidden=[128,64], contraction_levels=[0.05,0.05],
                pretraining_epochs=5, training_epochs=400,
    ...
    test_set_y_pred=deep_feat_select_ScA.test_model(classifier, test_set_x_org, batch_size=200)
