...
def train_model(train_set_x_org=None, train_set_y_org=None, 
    ...
    """
    Train a model of stacked contractive autoencoders. 
    ...
    sca=ScA(rng=rng, n_in=n_feat,
                 n_hidden=n_hidden, n_out=n_cl, 
                 lambda_reg=lambda_reg, alpha_reg=alpha_reg,batch_size=batch_size)
    # pretrain the model
    start_time=time.clock()
    pretrain_model(sca,train_set_x,pretrain_lr=pretrain_lr,pretraining_epochs=pretraining_epochs,
                   contraction_levels=contraction_levels,
                   batch_size=batch_size)
    # finetune
    finetune_model(sca,train_set_x=train_set_x, train_set_y=train_set_y, 
...
def test_model(classifier,test_set_x_org,batch_size=200):
    ...
    INPUTS:
    classifier_trained: object of ScA, the model learned by function "train_model". 
